...
class UpdateIFood extends Command
{
     ...
     * @var string
     */
    protected $signature = 'crawler:update-ifood
                            {--C|city=新北市 : 城市}
                            {--A|area=三重區 : 地區}';
    ...
    public function handle()
    {
        ...
        for ($page = $this->page; $page <= $totalPage; $page++) {
            ...
            $count = 0;
            $restaurants->each(function ($node) use (&$count) {
                $title = @$node->filterXPath('//div[contains(@class, "title-row")]//a')->text();
                if ($restaurant = \App\Models\Restaurant::where('title', $title)->first()) {
                    $categories = [];
                    ...
                    $image = @$node->filterXPath('//img')->evaluate('string(@data-src)')[0];
                    $restaurant->update([
                        'image' => !empty($image) ? $image : @$node->filterXPath('//img')->evaluate('string(@src)')[0],
                        'title' => @$node->filterXPath('//div[contains(@class, "title-row")]//a')->text(),
